...
@CommandLine.EntryPoint(
    delete_before=CommandLine.EntryPoint.Parameter("Delete files that are older than this value"),
)
@CommandLine.Constraints(
    delete_before=DurationTypeInfo(
        arity="?",
    ),
    output_stream=None,
)
def EntryPoint(
    delete_before=datetime.timedelta(
        days=7,
    ),
    yes=False,
            ...
            for filename in FileSystem.WalkFiles(
                ...
                file_infos.append(
                    FileInfo(
                        ...
                        filename,
                        datetime.timedelta(
                            seconds=t - os.stat(filename)[stat.ST_MTIME],
                        ),
        ...
        if not yes:
            ...
            dm.stream.write(
                textwrap.dedent(
                    ...
                    files="\n".join(
                        [
                            "    {name:<26}  {type:18}  {size:<17}  {age:<30}  {fullpath}".format(
                                name=fi.Name,
                                type=fi.Type,
        ...
        with dm.stream.DoneManager() as this_dm:
            for index, fi in enumerate(file_infos):
                this_dm.stream.write(
                    "Removing '{}' ({} of {})...".format(fi.Fullpath, index + 1, len(file_infos)),
                )
