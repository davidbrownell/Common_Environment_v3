...
# ----------------------------------------------------------------------
@CommandLine.EntryPoint
@CommandLine.Constraints( directory=CommandLine.DirectoryTypeInfo(),
                          output_stream=None,
                        )
def AllChangeStatus( directory,
                     output_stream=sys.stdout,
                     verbose=False,
                   ):
    ...
    def Query(directory, scm, task_index):
        ...
        if status:
            ...
            changes[task_index] = ChangeInfo( scm,
                                              directory,
                                              status,
    ...
    result = _WrapAll( "Detecting Changes",
                       Query,
                       None,
                       None,
                       directory,
    ...
    template = '  '.join([ "{{{}:<{}}}".format(index, col_size) for index, col_size in enumerate(six.itervalues(cols)) ])
    ...
    for change in changes:
        ...
        output_stream.write("{}\n".format(template.format( change.Directory,
                                                           change.Scm.Name,
                                                           change.Status.branch,
                                                           change.CurrentBranch,
                                                           str(change.Status.untracked) if change.Status.untracked is not None else "M/A",
                                                           str(change.Status.working) if change.Status.working is not None else "N/A",
                                                           str(change.Status.local) if change.Status.local is not None else "N/A",
                                                           str(change.Status.remote) if change.Status.remote is not None else "N/A",
                                                           str(change.Status.update) if change.Status.update is not None else "N/A",
                                                         )))
...
def AllWorkingChangeStatus( directory,
                            output_stream=sys.stdout,
                            verbose=False,
                          ):
    """Detects local and untracked changes."""
    ...
    result = _WrapAll( "Detecting Local Changes",
                       Query,
                       None,
                       None,
                       directory,
    ...
    if changed_repos:
        output_stream.write(textwrap.dedent(
            ...
            """).format( inflect.no("repository", len(changed_repos)),
                         '\n'.join([ "    - {} [{}]".format(directory, scm.Name) for scm, directory in changed_repos ]),
                       ))
...
def PullAndUpdateAll( directory,
                      output_stream=sys.stdout,
                      verbose=False,
                    ):
...
# ----------------------------------------------------------------------
def _WrapAll( query_method_name,
    ...
    with StreamDecorator(output_stream).DoneManager( line_prefix='',
        ...
        with dm.stream.DoneManager( done_suffix=lambda: "{} found".format(inflect.no("repository", len(repositories))),
                                  ):
        ...
        with dm.stream.SingleLineDoneManager( "Running...",
            ...
            this_dm.result = TaskPool.Execute( [ TaskPool.Task( repository[1],
                                                                Impl,
                                                              )
        ...
        if processed_repositories:
            dm.stream.write(textwrap.dedent(
                ...
                """).format( this=inflect.plural("This", len(processed_repositories)),
                             ...
                             results='\n'.join([ "    - {} [{}]".format(key, processed_repositories[key]) for key in sorted(six.iterkeys(processed_repositories)) ]),
                           ))
