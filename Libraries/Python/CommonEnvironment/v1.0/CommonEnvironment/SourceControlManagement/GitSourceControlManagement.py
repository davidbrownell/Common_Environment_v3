...
class GitSourceControlManagement(DistributedSourceControlManagement):
    ...
    def Clone(
        ...
        temp_clone_name = "{}.tmp".format(clone_name)

        FileSystem.MakeDirs(clone_path)

        result, content = cls.Execute(
            clone_path,
            'git clone{branch} "{uri}" "{name}"'.format(
        ...
        if result == 0:
            os.rename(os.path.join(clone_path, temp_clone_name), os.path.join(clone_path, clone_name))
    ...
    def Who(cls, repo_root):
        ...
        return "{} <{}>".format(GetValue("user.name"), GetValue("user.email"))
    ...
    def Clean(
        ...
        if not no_prompt and not cls.AreYouSurePrompt(
            textwrap.dedent(
                """\
    ...
    def GetMostRecentBranch(cls, repo_root):
        result, output = cls.Execute(repo_root, 'git for-each-ref --sort=-committerdate --format="%(refname)"')
        assert result == 0, (result, output)
    ...
    def CreateBranch(cls, repo_root, branch_name):
        return cls.Execute(
            repo_root,
            'git branch "{name}" && git checkout "{name}"'.format(
                name=branch_name,
            ),
        )
    ...
    def GetChangeInfo(cls, repo_root, change):
        ...
        d = {"user": lines[0].lstrip(), "date": lines[1].lstrip(), "summary": lines[2].lstrip()}
    ...
    def Update(
        ...
        elif isinstance(update_arg, (ChangeUpdateMergeArg, DateUpdateMergeArg, BranchAndDateUpdateMergeArg)):
    ...
    def GetChangesSinceLastMerge(
        ...
        if source_merge_arg is None or isinstance(source_merge_arg, EmptyUpdateMergeArg):
            ...
            additional_filters.append(
                '--{}="{}"'.format(GetDateOperator(source_merge_arg.GreaterThan), StringSerialization.SerializeItem(DateTimeTypeInfo(), source_merge_arg.Date)),
            )
        ...
        command_line = 'git --no-pager log "{source_branch}" --not "{dest_branch}" --format="%H" --no-merges{additional_filters}'.format(
    ...
    def Push(
        ...
        commands = ["git push", "git push --tags"]
    ...
    def Pull(
        ...
        for branch_name in branch_or_branches or []:
            if branch_name not in existing_branches:
                commands.append(
                    'git checkout -b "{name}" "origin/{name}"'.format(
                        name=branch_name,
                    ),
                )

        commands += ["git fetch --all", "git fetch --all --tags"]
    ...
    def _UpdateMergeArgToString(cls, repo_root, update_arg):
        ...
        def DateAndBranch(date, branch, operator):
            ...
            if branch:
                ...
                # ----------------------------------------------------------------------
                def BranchGenerator():
                    for branch in [cls.GetCurrentBranch(repo_root), cls.DefaultBranch]:
                        yield branch
