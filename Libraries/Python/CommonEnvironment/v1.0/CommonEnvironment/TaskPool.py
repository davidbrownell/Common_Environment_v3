...
def Transform(
    ...
    def Impl(task_index, on_status_update):
        transformed_items[task_index] = functor(OrderedDict([("item", items[task_index]), ("on_status_update", on_status_update)]))

    # ----------------------------------------------------------------------

    Execute(
        [Task(name_functor(index, item), Impl) for index, item in enumerate(items)],
        ...
        progress_bar_cols=progress_bar_cols,
    )
...
def Execute(
    ...
    progress_bar_cols = progress_bar_cols or 150
    ...
    # ----------------------------------------------------------------------
    def Invoke(
        get_status_functor,                             # def Func(future, task, update_type, optional_content) -> string
        ...
        with ThreadPoolExecutor(min(num_concurrent_tasks, len(tasks))) as executor:
            ...
            for future, task in six.moves.zip(futures, tasks):
                ...
                future.add_done_callback(
                    lambda ignore, future=future, task=task: UpdateStatus(future, task, StatusUpdate.Stop, None),
                )

            display_thread = threading.Thread(
                target=DisplayStatusesThreadProc,
            )
        ...
        with tqdm(
            ...
            def PBGetStatus(_, task, update_type, optional_content):
                ...
                if len(result) > progress_bar_cols:
                    ...
                    starting_index = int(progress_bar_cols / 2)

                    result = "{}...{}".format(result[:starting_index], result[starting_index + to_remove :])
...
class _InternalTask(Task):
    def __init__(self, task):
        super(_InternalTask, self).__init__(task.Name, Interface.CreateCulledCallable(task.Functor))
