                        ...
                        for index, configuration in enumerate(repository_activation_configurations):
                            ...
                            with all_activated_dm.stream.DoneManager(suffix='\n') as activated_dm:
                                ...
                                with activated_dm.stream.DoneManager(suffix='\n') as this_dm:
                                    command_line = 'docker run -t --name "{container_name}" "{image_name}:base" /sbin/my_init -- /sbin/setuser "{username}" bash -c "cd {image_code_dir} && . ./Activate.sh {configuration} && pushd {image_code_base}/Common/Environment && python -m RepositoryBootstrap.EnvironmentDiffs After /decorate' \
