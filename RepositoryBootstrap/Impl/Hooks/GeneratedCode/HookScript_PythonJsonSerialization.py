...
# |  Generated on: 2019-02-23 22:15:39.397249
# |
# --------------------------------------------------------------------------------
import copy
import sys
...
def Deserialize(
    ...
    if isinstance(root, six.string_types):
        if len(root) < 2000 and os.path.isfile(root):
            with open(root) as f:
                root = json.load(f)
        else:
...
def Deserialize_Commit(
    ...
    if isinstance(item, six.string_types):
        if len(item) < 2000 and os.path.isfile(item):
            with open(item) as f:
                item = json.load(f)
        else:
...
class Serializer(object):
    ...
    def _Pushed_Item(cls, item, always_include_optional, process_additional_data):
        ...
        # change
        try:
            setattr(result, "change", cls.Pushed_change(
                cls._GetPythonAttribute(
                    item,
                    "change",
                    is_optional=False,
