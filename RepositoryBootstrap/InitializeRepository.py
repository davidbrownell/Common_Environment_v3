...
def EntryPoint(
    ...
    with StreamDecorator(output_stream).DoneManager(
        ...
        support_git = _Prompt("Include .gitignore for Git support? ", "yes").lower() in [
            "yes",
            "y",
        ]
        support_hg = _Prompt("Include .hgignore for Hg support? ", "yes").lower() in [
            "yes",
            "y",
        ]
        support_windows = _Prompt("Support development on Windows? ", "yes").lower() in [
            "yes",
            "y",
        ]
        ...
        support_linux = _Prompt("Support development on Linux? ", "yes").lower() in [
            "yes",
            "y",
        ]

        include_boost_license = _Prompt(
            "Include the boost software license (https://www.boost.org/users/license.html)? ",
            "no",
        ).lower() in ["yes", "y"]

        include_bootstrap = _Prompt("Include bootstrap scripts? ", "no").lower() in [
            "yes",
            "y",
        ]
        ...
        with dm.stream.DoneManager() as this_dm:
            ...
            dest_filename = os.path.join(
                repo_dir,
                RepositoryBootstrapConstants.REPOSITORY_ID_FILENAME,
            )
            if not os.path.exists(dest_filename):
                with open(dest_filename, "w") as f:
                    f.write(
                        RepositoryBootstrapConstants.REPOSITORY_ID_CONTENT_TEMPLATE.format(
                            name=repo_name,
        ...
        dm.stream.write(
            textwrap.dedent(
                ...
                **********************************************************************
                ...
                setup=os.path.join(
                    repo_dir,
                    RepositoryBootstrapConstants.SETUP_ENVIRONMENT_CUSTOMIZATION_FILENAME,
                ),
                activate=os.path.join(
                    repo_dir,
                    RepositoryBootstrapConstants.ACTIVATE_ENVIRONMENT_CUSTOMIZATION_FILENAME,
                ),
                scm_hook=os.path.join(
                    repo_dir,
                    RepositoryBootstrapConstants.HOOK_ENVIRONMENT_CUSTOMIZATION_FILENAME,
                ),
                ...
                bootstrap="" if not include_bootstrap else StringHelpers.LeftJustify(
                    textwrap.dedent(
                        ...
                        """,
                    ).format(os.path.join(repo_dir, "bootstrap_impl.py")),
                    4,
                ),
            ),
        )
