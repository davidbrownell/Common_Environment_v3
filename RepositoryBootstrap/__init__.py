...
# ----------------------------------------------------------------------
def GetRepoMapFromSetup( repository_root,
                         max_repo_search_depth=None,
                         required_ancestor_dir=None,
                       ):
    """Returns a repository map generated by calling a Setup file's List method"""

    from RepositoryBootstrap import Constants
    from RepositoryBootstrap.Impl import CommonEnvironmentImports

    result, output = CommonEnvironmentImports.Process.Execute("{} List /recurse /json /decorate{}{}" \
                        .format( os.path.join(repository_root, CommonEnvironmentImports.CurrentShell.CreateScriptName(Constants.SETUP_ENVIRONMENT_NAME)),
    ...
    class Value(object):
        ...
        def __init__(self, json_item):
            ...
            self.Dependencies               = OrderedDict([ ( None if k == "<None>" else k, [ tuple(item) for item in v ] ) for k, v in six.iteritems(json_item["dependencies"]) ])
            self.Dependents                 = OrderedDict([ ( None if k == "<None>" else k, [ tuple(item) for item in v ] ) for k, v in six.iteritems(json_item["dependents"]) ])
